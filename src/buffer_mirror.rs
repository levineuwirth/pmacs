...
impl BufferMirror {
    ...
    // E6c: the frontend-side undo arm (`apply_local_undo`) is
    // retired, and this is the record of why it is gone rather than
    // a method. M10.11 P1 undid through each mirror's own peer-bound
    // loro `UndoManager` and sent the inverse op upstream — but the
    // GPU never built that arm, single-key chords were its whole
    // reach, and it raced the daemon's undo over the same op. Undo
    // is one daemon-side mechanism now (the cross-peer arbiter on
    // `Buffer`: per-source groups, daemon-peer compensation), and
    // every undo chord round-trips to it. The mirror keeps its
    // `CrdtState` for optimistic inserts/deletes and remote-op
    // application; `CrdtState` carries no `UndoManager` any more.
