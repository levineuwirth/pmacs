...
| **the structural difference worth testing next — ~~CANDIDATE~~ WITHDRAWN, FALSE PREMISE** | ~~`cargo test --workspace` runs **many test binaries concurrently**; `--lib` runs **one**.~~ **The observed run does not do this.** Under **cargo 1.95.0** the `08-sweep` step ran its test binaries **sequentially**: strict `Running` → `test result` → `Running` alternation across 119 binaries (U10, same tree). The sweep and `--lib` ran the **same libtest target** — 1,940-test population, default intra-binary threading — so there was no concurrency difference between the steps — the "difference in kind" this row proposed does not exist, and no experiment was needed to retire it. **What survives is the second half of the control, still unrun:** run the `--lib` binary alone under **synthetic load**. That remains the useful experiment for this family |
...
**Another intermittent occurrence with an in-run control, on a
documentation-only diff.** U9 red in the sweep and green in `--lib`;
this occurrence red in `--lib` and green in the sweep — **the same
run**, minutes apart.

**The step names tempt a concurrency reading. Do not take it**, and
this row's own first draft did: both steps ran the **same libtest
target** — same 1,940-test population, same default intra-binary
threading — so the pair establishes intermittence and nothing about
concurrency. (Same *target*, not same *artifact*: the two binaries
differ in bytes. See the precision row below.) See also U9.
...
| **what it does to U9's named next test** | **U9's candidate is withdrawn because its PREMISE IS FALSE, not because this occurrence reversed it.** U9 supposed that `--workspace` runs **many test binaries concurrently** while `--lib` runs one. Under **cargo 1.95.0** this run's sweep executed its binaries **sequentially**: strict `Running` → `test result` → `Running` alternation across **119** of them, with the failing test in the **first**. Both steps ran the **same libtest target** — 1,940-test population, default intra-binary threading (`03-lib.log:3`, `08-sweep.log:3`). **There was no concurrency difference between the two steps to compare.** U9's structural candidate was never a real variable **in the observed execution**; this row does not generalize to other cargo versions or invocations it did not measure |
| **precision, because the tempting shorthand is wrong** | the two steps ran the same **target**, NOT the same **artifact**. `03-lib` ran `pmacs-9d4963151c6f3151` (211,777,472 bytes) and the sweep ran `pmacs-7624ed4bed0b6617` (214,462,056 bytes) — **different hashes, different sizes, `cmp` reports differing bytes**. The supported claim is the target, its 1,940-test population and its threading; "the same executable" is not supported and is not made |
| **the claim this row FIRST MADE AND WITHDREW** | that U10 "runs the comparison in the opposite direction" — red at low concurrency, green at high. **False.** It presupposed exactly the premise above. An experiment cannot reverse a variable that was never varied. Corrected in place rather than deleted, because the wrong version is the one a later reader would otherwise reconstruct from the two step names |
| **what remains untested** | synthetic load against the `--lib` binary alone. U9 named it; it is still the useful experiment; neither U9 nor U10 has run it |
