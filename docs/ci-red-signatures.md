...
| **the structural difference worth testing next — ~~CANDIDATE~~ WITHDRAWN, FALSE PREMISE** | ~~`cargo test --workspace` runs **many test binaries concurrently**; `--lib` runs **one**.~~ **Cargo does not do this.** It runs test binaries **sequentially**: a `08-sweep` log shows strict `Running` → `test result` → `Running` alternation across 119 binaries (U10, same tree). The sweep and `--lib` run the **same** `pmacs` libtest executable with the **same** default intra-binary threading, so there is no concurrency difference between the steps at all — the "difference in kind" this row proposed does not exist, and no experiment was needed to retire it. **What survives is the second half of the control, still unrun:** run the `--lib` binary alone under **synthetic load**. That remains the useful experiment for this family |
...
this row's own first draft did: the two steps ran the **same libtest
executable** with the **same** threading, so the pair establishes
intermittence and nothing about concurrency. See the U9 row below.
...
| **what it does to U9's named next test** | **U9's candidate is withdrawn because its PREMISE IS FALSE, not because this occurrence reversed it.** U9 supposed that `--workspace` runs **many test binaries concurrently** while `--lib` runs one. Cargo does not: it runs test binaries **sequentially**. This run's sweep log shows strict `Running` → `test result` → `Running` alternation across **119** binaries, and the failing test is in the **first** one — the same 1,940-test `pmacs` libtest executable that `03-lib` ran (`03-lib.log:3`, `08-sweep.log:3`), with the same default intra-binary threading. **There was no concurrency difference between the two steps to compare.** U9's structural candidate was never a real variable |
