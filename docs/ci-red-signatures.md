...
| occurrences | at least eight: three on 2026-08-31 within eleven hours, the third on `main` after a documentation-only merge; three more on 2026-09-08 in local sweeps within an hour (gate logs `20260908T101700Z-2279879` and `20260908T105335Z-2697918`, one selector each, and `20260908T110408Z-2820743`, FIVE together --- `cache_survives_across_fetcher_instances`, `fetch_after_upstream_tag_removed_surfaces_at_resolve`, `fetch_clones_into_cache`, `fetch_twice_does_not_reclone_via_sentinel`, `resolve_branch_returns_branch_head` --- all carrying both fragments), on a branch touching neither `packages` nor `file_io`. A seventh followed about five hours later, in review round 1's own six-stage gate at the branch head `e78d184` (gate log `20260908T154623Z-3116795`, step `05-sweep`): `cache_survives_across_fetcher_instances` alone, `src/packages/fetcher.rs:929`, both fragments, `test result: FAILED. 2199 passed; 1 failed; 11 ignored`. That is the first on the branch head and the first the widened selector caught on its next try. An eighth about forty minutes after it, in fix round 1's seven-stage gate at `68a9767` (gate log `20260908T163205Z-3389418`, step `05-sweep`): TWO selectors this time, `cache_survives_across_fetcher_instances` at `src/packages/fetcher.rs:929` and `fetch_after_upstream_tag_removed_surfaces_at_resolve` at `:1021`, both carrying both fragments, `test result: FAILED. 2198 passed; 2 failed; 11 ignored`; step `06-sweep-luajit` of the same run is clean, 124 targets 4271/0/35, so the Lua flavor is not the discriminator here either. A ninth five minutes after the eighth, in the very next gate run at `d0771fc` (gate log `20260908T164713Z-3615153`) and this time in step `06-sweep-luajit`, on THREE selectors --- the eighth's two plus `fetch_clones_into_cache` at `src/packages/fetcher.rs:889` --- all with both fragments, `test result: FAILED. 2004 passed; 3 failed; 10 ignored`, while step `05-sweep` of that same run is CLEAN at 124 targets 4571/0/49. The eighth and the ninth are mirror images one run apart, so the sweep flavor is not the discriminator: the row is about the workspace sweep, either flavor. Several intervening sweeps of the same tree were green, so the rate on this machine is neither zero nor one. Five occurrences on 2026-09-08 alone, every one on a tree touching neither `packages` nor `file_io`, is the strongest argument yet for doing the retirement work rather than counting further |
